    ...
    m_error(QBluetoothTransferReply::NoError), m_errorStr(), m_transfer_path()
{
    ...
    if (!input) {
        qDebug() << "Invalid input device (null)";
    ...
    if (isBluez5()) {
        ...
        if (!QDBusConnection::sessionBus().registerObject(m_agent_path, this))
            qDebug() << "Failed creating obex agent dbus objects";
    }
...
bool QBluetoothTransferReplyBluez::start()
{
    ...
    if(!file){
        ...
        m_tempfile->open();
        qDebug() << "Not a QFile, making a copy" << m_tempfile->fileName();
...
void QBluetoothTransferReplyBluez::cleanupSession()
{
    ...
    if (reply.isError())
        qDebug() << "Abort: Cannot remove obex session";
...
void QBluetoothTransferReplyBluez::sessionCreated(QDBusPendingCallWatcher *watcher)
{
    ...
    if (reply.isError()) {
        qDebug() << "Failed to create obex session:"
...
void QBluetoothTransferReplyBluez::sessionStarted(QDBusPendingCallWatcher *watcher)
{
    ...
    if (reply.isError()) {
        qDebug() << "Failed to start obex session:"
                               << reply.error().name() << reply.reply().errorMessage();
                                                  ...
                                                  const QStringList &)
{
    ...
    qDebug() << "Transfer update:" << interface << changed_properties;
}
...
void QBluetoothTransferReplyBluez::abort()
{
    ...
    if (m_client) {
        ...
        reply.waitForFinished();
        if (reply.isError())
            qDebug() << "Failed to abort transfer" << reply.error().message();
